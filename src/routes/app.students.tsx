...
function StudentsPage() {
  ...
  const derivedStudents = useMemo(() => {
    return data.students.map((student, index) => {
      ...
      const academicScore = scores.length ? Math.round(scores.reduce((sum, s) => sum + s, 0) / scores.length) : 0;
      const feeStatusDisplay = student.feeStatus === "Overdue" && index % 5 !== 0
        ? "Pending"
        : student.feeStatus;
      ...
      const needsAttention = student.attendance < 85 || (academicScore > 0 && academicScore < 75) || feeStatusDisplay === "Pending";
      const risk: "Healthy" | "Needs Attention" | "At Risk" = attendanceRisk || academicRisk || feeRisk
        ? "At Risk"
        : needsAttention
        ? "Needs Attention"
        : "Healthy";
      const attendanceBandValue: AttendanceBand = student.attendance >= 90
        ? "90+"
        : student.attendance >= 75
        ? "75-89"
        : "below-75";
      const academicBandValue: AcademicBand = academicScore >= 80
        ? "80+"
        : academicScore >= 65
        ? "65-79"
        : "below-65";
  ...
  const selectedStudent = derivedStudents.find((student) => student.id === selectedStudentId) || null;
          ...
          <Card className="p-3 sm:p-4">
            <div className="flex flex-wrap gap-2 sm:gap-3 items-center">
              ...
              <Select value={academicBand} onValueChange={(v: AcademicBand) => { setAcademicBand(v); setPage(1); }}>
                <SelectTrigger className="w-44"><SelectValue placeholder="Academic" /></SelectTrigger>
                <SelectContent>
              ...
              <Select value={admissionYear} onValueChange={(v) => { setAdmissionYear(v); setPage(1); }}>
                <SelectTrigger className="w-36"><SelectValue placeholder="Admission Year" /></SelectTrigger>
                <SelectContent>
              ...
              <Button variant="outline" size="sm" onClick={clearFilters}>
                <Filter className="size-4 mr-1" />Clear Filters
              </Button>
                ...
                <TableBody>
                  {rows.map((student) => (
                    <TableRow key={student.id} className="cursor-pointer" onClick={() => setSelectedStudentId(student.id)}>
                      ...
                      <TableCell className={student.attendance >= 90 ? "text-emerald-600 font-medium" : student.attendance >= 75 ? "text-amber-600 font-medium" : "text-red-600 font-medium"}>
                        {student.attendance}%
                      </TableCell>
                      <TableCell className={student.academicScore >= 80 ? "text-emerald-600 font-medium" : student.academicScore >= 65 ? "text-amber-600 font-medium" : "text-red-600 font-medium"}>
                        {student.academicScore || "-"}
                      </TableCell>
                      <TableCell>
                        <Badge variant="secondary" className={student.feeLabel === "Paid" ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300" : student.feeLabel === "Overdue" ? "bg-red-100 text-red-700 dark:bg-red-950 dark:text-red-300" : "bg-amber-100 text-amber-700 dark:bg-amber-950 dark:text-amber-300"}>
                          {student.feeLabel}
                        </Badge>
                      </TableCell>
                      <TableCell>
                  ...
                  <Tabs defaultValue="overview" className="w-full">
                    ...
                    <TabsContent value="attendance">
                      <Card className="p-3 sm:p-4 space-y-4">
                        ...
                        <div>
                          ...
                          <Badge className={selectedStudent.attendance >= 90 ? "bg-emerald-100 text-emerald-700" : selectedStudent.attendance >= 75 ? "bg-amber-100 text-amber-700" : "bg-red-100 text-red-700"}>
                            {selectedStudent.attendance >= 90 ? "Excellent" : selectedStudent.attendance >= 75 ? "Good" : "Needs Attention"}
                          </Badge>
                        </div>
                        <div className="text-xs text-muted-foreground">Monthly trend: steady with minor dips last week.</div>
                      </Card>
                    </TabsContent>
                        ...
                        <div className="space-y-2 text-xs">
                          {selectedStudent.testScores ? Object.entries(selectedStudent.testScores).map(([subject, score]) => (
                            <div key={subject} className="flex items-center justify-between">
                              <span>{subject}</span>
                              <span className={score >= 80 ? "text-emerald-600" : score >= 65 ? "text-amber-600" : "text-red-600"}>{score}%</span>
                            </div>
                          )) : <span className="text-muted-foreground">No subject scores available.</span>}
                        </div>
                        <div className="text-xs text-muted-foreground">Weak subjects: {getWeakSubjects(selectedStudent).join(", ") || "None"}</div>
                      </Card>
                    </TabsContent>
                    ...
                    <TabsContent value="fees">
                      <Card className="p-3 sm:p-4 space-y-4 text-sm">
                        ...
                        <ProfileItem label="Concession" value={Number(selectedStudent.roll) % 3 === 0 ? "Sibling" : "None"} />
                      </Card>
                    </TabsContent>
                    ...
                    <TabsContent value="communication">
                      <Card className="p-3 sm:p-4 space-y-3 text-sm">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">4 parent messages logged.</p>
                        </div>
                        <div>
                          <p className="font-medium">Communication History</p>
                          <p className="text-xs text-muted-foreground">Last contact: 2026-05-20</p>
                        </div>
                      </Card>
                    </TabsContent>
                  </Tabs>
