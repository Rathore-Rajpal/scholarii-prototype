...
} from "@/lib/scholarii/principal-messages-mock-data";
...
function MessagesPage() {
  ...
  const FILTER_OPTIONS = isSchoolStaff
    ? PRINCIPAL_FILTER_OPTIONS
    : isTeacher
      ? TEACHER_FILTER_OPTIONS
      : STUDENT_FILTER_OPTIONS;
  const [conversations, setConversations] = useState<Conversation[]>(
    isSchoolStaff ? PRINCIPAL_CONVERSATIONS : isTeacher ? TEACHER_CONVERSATIONS : CONVERSATIONS,
  );
  ...
  useEffect(() => {
    localStorage.setItem(
      "scholarii-msg-panel",
      JSON.stringify({ width: panelWidth, collapsed: panelCollapsed }),
    );
  ...
  const filtered = conversations
    .filter((c) => {
      if (filter === "Teachers")
        return c.type === "dm" && (c.role === "Teacher" || c.role === "HOD");
      if (filter === "Students")
        return (
          c.type === "dm" &&
          !c.role &&
          c.subject !== "Parent" &&
          c.subject !== "Administration" &&
          c.subject !== "Staff"
        );
  ...
  const recentTeachers = CONVERSATIONS.filter((c) => c.type === "dm" && c.role === "Teacher").slice(
    0,
    3,
  );
  ...
  return (
    <div className="flex h-screen flex-col bg-background">
      ...
      <header className="flex items-center justify-between border-b border-border/40 bg-background/80 px-3 py-2 backdrop-blur-xl sm:px-4 sm:py-2.5">
        <div className="flex min-w-0 items-center gap-2 sm:gap-3">
          <button
            ...
            className="shrink-0 rounded-lg p-1.5 text-muted-foreground hover:bg-muted hover:text-foreground sm:p-2 lg:hidden"
          >
          ...
          {!selected && <span className="text-sm font-bold text-foreground">Messages</span>}
        </div>
      ...
      <div className="flex flex-1 overflow-hidden">
              ...
              {filtered
                ...
                .map((conv) => (
                  ...
                  >
                    <div className="relative shrink-0">
                      <div
                        className={`flex items-center justify-center rounded-full bg-muted/60 text-lg ${panelCollapsed ? "h-9 w-9" : "h-10 w-10"}`}
                      >
                        {conv.avatar}
                      </div>
                      {conv.online && (
            ...
            <div className="border-t border-border/40 p-1.5">
              ...
              {filtered
                ...
                .map((conv) => (
                  ...
                  >
                    <div
                      className={`flex shrink-0 items-center justify-center rounded-xl bg-muted/60 text-lg ${panelCollapsed ? "h-9 w-9" : "h-10 w-10"}`}
                    >
                      {conv.avatar}
                    </div>
                    {!panelCollapsed && (
            ...
            <>
              ...
              <div className="flex items-center justify-between border-b border-border/40 bg-background/80 px-3 py-3 backdrop-blur-xl sm:px-4">
                <div className="flex items-center gap-3">
                  ...
                  <div className="relative">
                    <div className="flex h-10 w-10 items-center justify-center rounded-full bg-muted/60 text-lg">
                      {selected.avatar}
                    </div>
              ...
              {!selected.readOnly ? (
                ...
                <div className="border-t border-border/40 bg-muted/20 px-3 py-3 text-center sm:px-4">
                  <p className="text-xs text-muted-foreground">
                    This channel is read-only. Only teachers and admins can post.
                  </p>
                </div>
              )}
            </>
        ...
        {showContext && (
          <aside
            className={`${mobileView === "chat" ? "hidden" : "hidden"} w-72 shrink-0 border-l border-border/40 bg-background/95 xl:block`}
          >
            {selected ? (
      ...
      <div className="max-w-lg w-full space-y-8">
        ...
        <div className="text-center space-y-3">
          ...
          <h2 className="text-base font-bold text-foreground sm:text-lg">
            Welcome to Scholarii Messages
          </h2>
          <p className="text-sm text-muted-foreground">
            Connect with teachers, classmates, and school communities.
          </p>
        </div>
  ...
  return (
    <div className="flex h-full flex-col p-4 space-y-4 overflow-y-auto">
      <div className="flex flex-col items-center space-y-3 py-4">
        <div className="relative">
          <div className="flex h-16 w-16 items-center justify-center rounded-full bg-muted/60 text-3xl">
            {conversation.avatar}
          </div>
        ...
        <div className="space-y-2">
          ...
          <div className="space-y-1.5">
            {[
              ...
              <div
                key={name}
                className="flex items-center gap-2 rounded-lg px-2 py-1.5 text-sm text-muted-foreground hover:bg-muted"
              >
    ...
    <div className="flex h-full flex-col p-4 space-y-5 overflow-y-auto">
      <div className="space-y-2">
        ...
        {recentTeachers.map((t) => (
          ...
          >
            <div className="relative shrink-0">
              <div className="flex h-8 w-8 items-center justify-center rounded-full bg-muted/60 text-sm">
                {t.avatar}
              </div>
              {t.online && (
                <div className="absolute -bottom-0.5 -right-0.5 h-2 w-2 rounded-full border border-background bg-emerald-500" />
              )}
            </div>
            <div className="min-w-0">
              <p className="text-xs font-medium text-foreground truncate">{t.name}</p>
      ...
      <div className="space-y-2">
        ...
        <div className="space-y-1.5">
          {["Algebra Notes Chapter 5", "Science Project Guide", "English Grammar Guide"].map(
            (r) => (
              <div
                key={r}
                className="flex items-center gap-2 rounded-lg border border-border/40 bg-muted/30 px-2.5 py-2"
              >
  ...
  return (
    <div className={`flex ${isOwn ? "justify-end" : "justify-start"}`}>
      <div className={`max-w-[75%] space-y-1 ${isOwn ? "text-right" : ""}`}>
        ...
        <div
          className={`rounded-2xl px-3 py-2 text-sm leading-relaxed sm:px-4 sm:py-2.5 ${
            isOwn
        ...
        >
          {message.type === "resource" ? (
            <div className="flex items-center gap-2">
              <FileText
                className={`h-4 w-4 shrink-0 ${isOwn ? "text-primary-foreground/70" : "text-blue-400"}`}
              />
              <div>
            ...
            <div className="flex items-center gap-2">
              <LinkIcon
                className={`h-4 w-4 shrink-0 ${isOwn ? "text-primary-foreground/70" : "text-cyan-400"}`}
              />
