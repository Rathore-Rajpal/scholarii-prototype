  ...
  return (
    <div className="space-y-5 sm:space-y-6">
      ...
      <Card className="p-4 sm:p-5">
        ...
        <p className="text-xs sm:text-sm text-muted-foreground mb-3 sm:mb-4">Annual fee payment status</p>
        <div className="space-y-2.5 sm:space-y-3">
          <div className="flex items-center justify-between text-xs sm:text-sm">
            <span className="text-muted-foreground">Progress</span>
            <span className="font-bold text-lg sm:text-xl">{data.paidPercentage}%</span>
          </div>
      ...
      <Card className="p-4 sm:p-5">
        ...
        <p className="text-xs sm:text-sm text-muted-foreground mb-3 sm:mb-4">Annual fee structure</p>
        <div className="space-y-2.5 sm:space-y-3">
          {data.feeBreakdown.map((fee) => {
            ...
            return (
              <div key={fee.category} className="flex items-center justify-between p-2.5 sm:p-3 rounded-xl border border-border/50">
                <div className="flex items-center gap-2.5 sm:gap-3">
                  <div className="rounded-lg bg-violet-500/10 p-1.5 sm:p-2">
                    <Icon className="size-3.5 sm:size-4 text-violet-600" />
                  </div>
                  <span className="text-xs sm:text-sm font-medium">{fee.category}</span>
                </div>
                <span className="text-xs sm:text-sm font-bold">₹{fee.amount.toLocaleString()}</span>
              </div>
            );
          })}
          <Separator className="my-2" />
          <div className="flex items-center justify-between p-2.5 sm:p-3">
            <span className="text-xs sm:text-sm font-semibold">Total Annual Fees</span>
            <span className="text-base sm:text-lg font-bold text-violet-600">₹{data.annualFees.toLocaleString()}</span>
          </div>
        </div>
      </Card>
    </div>
  ...
  return (
    <div className="space-y-5 sm:space-y-6">
      <Card className="p-4 sm:p-5">
        ...
        <p className="text-xs sm:text-sm text-muted-foreground mb-3 sm:mb-4">Record of all fee payments</p>
        <div className="space-y-2.5 sm:space-y-3">
          {data.paymentHistory.map((payment) => (
            <div key={payment.id} className="flex items-center justify-between p-3 sm:p-4 rounded-xl border border-border/50 transition-all hover:shadow-md">
              <div className="flex items-center gap-2.5 sm:gap-3">
                <div className="rounded-lg bg-emerald-500/10 p-1.5 sm:p-2">
                  <CheckCircle2 className="size-3.5 sm:size-4 text-emerald-600" />
                </div>
                <div>
                  <p className="text-xs sm:text-sm font-medium">{payment.installment}</p>
                  <p className="text-[10px] sm:text-xs text-muted-foreground">{payment.paidDate} · {payment.paymentMethod}</p>
                </div>
              </div>
              <div className="text-right shrink-0">
                <p className="text-xs sm:text-sm font-bold">₹{payment.amount.toLocaleString()}</p>
  ...
  return (
    <div className="space-y-5 sm:space-y-6">
      <Card className="p-4 sm:p-5">
        ...
        <p className="text-xs sm:text-sm text-muted-foreground mb-3 sm:mb-4">Installments due soon</p>
        <div className="space-y-3 sm:space-y-4">
          {data.upcomingPayments.map((payment) => (
            <div key={payment.id} className="p-4 sm:p-5 rounded-xl border border-border/50 bg-gradient-to-br from-amber-500/5 to-transparent transition-all hover:shadow-md">
              <div className="flex items-start justify-between">
                <div className="space-y-1.5 sm:space-y-2">
                  <p className="text-base sm:text-lg font-bold">{payment.installment}</p>
                  <p className="text-xl sm:text-2xl font-bold text-amber-600">₹{payment.amount.toLocaleString()}</p>
                  <div className="flex items-center gap-2">
                    <CalendarDays className="size-3 sm:size-3.5 text-muted-foreground" />
                    <span className="text-xs sm:text-sm text-muted-foreground">Due: {payment.dueDate}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Clock className="size-3 sm:size-3.5 text-amber-600" />
                    <span className="text-xs sm:text-sm font-medium text-amber-600">{payment.daysRemaining} Days Remaining</span>
                  </div>
                </div>
                <Badge variant="outline" className="text-[10px] sm:text-xs bg-amber-500/10 text-amber-600 border-amber-500/20">
                  High Priority
                </Badge>
              </div>
              <Button className="w-full mt-3 sm:mt-4 bg-gradient-to-r from-amber-500 to-orange-600 text-white hover:from-amber-600 hover:to-orange-700">
  ...
  return (
    <div className="space-y-5 sm:space-y-6">
      <Card className="p-4 sm:p-5">
        ...
        <p className="text-xs sm:text-sm text-muted-foreground mb-3 sm:mb-4">Download payment receipts for your records</p>
        <div className="space-y-2.5 sm:space-y-3">
          {data.receipts.map((receipt) => (
            <div key={receipt.id} className="flex items-center justify-between p-3 sm:p-4 rounded-xl border border-border/50 transition-all hover:shadow-md">
              <div className="flex items-center gap-2.5 sm:gap-3">
                <div className="rounded-lg bg-blue-500/10 p-1.5 sm:p-2">
                  <FileText className="size-3.5 sm:size-4 text-blue-600" />
                </div>
                <div>
                  <p className="text-xs sm:text-sm font-medium">{receipt.receiptNumber}</p>
                  <p className="text-[10px] sm:text-xs text-muted-foreground">{receipt.month} · ₹{receipt.amount.toLocaleString()}</p>
                </div>
              </div>
...
function FeesPage() {
  ...
  return (
    <div className="space-y-5 sm:space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold tracking-tight">Fees</h1>
      ...
      <div className="grid grid-cols-2 gap-3 sm:gap-4">
        ...
        <KpiCard
          ...
          trendLabel={`${data.paidPercentage}% Paid`}
          tone="success"
        />
        <KpiCard
          icon={AlertTriangle}
          ...
          trendLabel={`${data.pendingPercentage}% Remaining`}
          tone="warning"
        />
        <KpiCard
          icon={CalendarDays}
          ...
          trendLabel={`${data.nextDueDays} Days Remaining`}
          tone="info"
        />
      </div>
      ...
      <Card className="relative overflow-hidden p-4 sm:p-5 border-violet-500/20">
        ...
        <div className="relative">
          <div className="flex items-center gap-2 mb-3 sm:mb-4">
            ...
            <div>
              <h3 className="font-semibold text-sm sm:text-base">AI Fee Insights</h3>
              <p className="text-[10px] sm:text-xs text-muted-foreground">Smart recommendations for your fee payments</p>
            </div>
          </div>
      ...
      <Card className="p-4 sm:p-5">
        ...
        <div className="grid grid-cols-2 gap-2.5 sm:gap-3">
          <Button variant="outline" className="h-auto py-3 sm:py-4 flex flex-col items-center gap-1.5 sm:gap-2">
            <Download className="size-4 sm:size-5 text-violet-600" />
            <span className="text-[10px] sm:text-xs font-medium">Download Receipt</span>
          </Button>
          <Button variant="outline" className="h-auto py-3 sm:py-4 flex flex-col items-center gap-1.5 sm:gap-2">
            <FileText className="size-4 sm:size-5 text-blue-600" />
            <span className="text-[10px] sm:text-xs font-medium">View Fee Structure</span>
          </Button>
          <Button variant="outline" className="h-auto py-3 sm:py-4 flex flex-col items-center gap-1.5 sm:gap-2">
            <BarChart3 className="size-4 sm:size-5 text-emerald-600" />
            <span className="text-[10px] sm:text-xs font-medium">Payment Timeline</span>
          </Button>
          <Button
            variant="outline"
            className="h-auto py-3 sm:py-4 flex flex-col items-center gap-1.5 sm:gap-2"
            onClick={() => nav({ to: "/app/ai" as never })}
          >
            <Sparkles className="size-4 sm:size-5 text-amber-600" />
            <span className="text-[10px] sm:text-xs font-medium">Open AI Assistant</span>
          </Button>
        </div>
      </Card>
    </div>
